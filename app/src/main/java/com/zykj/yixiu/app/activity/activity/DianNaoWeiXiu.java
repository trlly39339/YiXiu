...
public class DianNaoWeiXiu extends BaseActivity {
    ...
    List<DianNaoFenLeiBean> listfl;//品牌的数据源
...
//类型选择————————————————————————————————————————————————————————————
                if (index==-1){
                    ...
                    map.put("category","1");
                    map.put("pid",listfl.get(index).getId()+"");
...
//                        成功刷新进度条fl
                            StyledDialog.dismissLoading();
                            if (Y.getRespCode(result)) {
                                listfl = JSON.parseArray(Y.getData(result), DianNaoFenLeiBean.class);
//                            创建选择器
                                OptionsPickerView opv = new OptionsPickerView.Builder(DianNaoWeiXiu.this, new OptionsPickerView.OnOptionsSelectListener() {
                                    @Override
                                    public void onOptionsSelect(int options1, int options2, int options3, View v) {
//                                    选择后的监听器
                                        ...
                                        tvLeixingXianshi.setText(listdn.get(options1).getName());//把解析出来的数据设置到控件里
                                        index = options1;//当前的索引
...
//                            把list进行转换
                                List<String> strings = new ArrayList<String>();
                                for (DianNaoFenLeiBean mb : listfl) {
                                    strings.add(mb.getName());
                                }
//                            添加数据
                                opv.setPicker(strings, null, null);
...
//                                    选择后的监听器
                                        ...
                                        tvXinghaoXianshi.setText(listdn.get(options1).getName());//把解析出来的数据设置到控件里
                                        index = options1;//当前的索引
                                    }
                                }).build();
//                            把list进行转换
